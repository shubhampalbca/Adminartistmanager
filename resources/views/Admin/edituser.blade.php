            ...
            <div class="container-fluid px-4">
               ...
               <div class="card mb-4">
                  <div class="card-header d-flex align-items-center justify-content-between">
                     <span><i class="fas fa-table me-1"></i>Edit User</span>
                     <a href="{{ url('adduser') }}" class="btn btn-secondary btn-sm">Back to Users</a>
                     <form action="{{ url('update-user/'.$users->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <select id="category" name="category" class="form-control">
                           ...
                           <option value="{{ $cat->categoryname }}" {{ (isset($users->category) && $users->category == $cat->categoryname) ? 'selected' : '' }}>{{ $cat->categoryname }}</option>
                           @endforeach
                        </select>
                        @endif
                        ...
                        <img src="{{ asset('uploads/profile/' . $users->profile) }}" width="50px" height="50px" alt="Profile"><br><br>
                        @endif
